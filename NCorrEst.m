function data = NCorrEst(tpl,tar,st,kn)
    ...
    hx = kn.hx; hy = kn.hy;
    % 3/3 is the width of each kernal
    % measures the center of each kernal so,
...
function [x,y,kn] = locs(kn)
    ...
    if rx>h || cy>w
        % these define between will search area
        error('number of elements in rx/cy needs to be larger(smaller-NS) than h/w');
    end
